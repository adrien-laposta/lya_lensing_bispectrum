...
class LyaForest:
    ...
    def __init__(self, z, use_aip15_extrapolation = False):
        ...
        # Load AIP+15 extrapolation tables
        self.aip15_extrapolation = pickle.load(open("aip15_parameter_extrapolation/aip15_fit_results.pkl", "rb"))

        # Initialize P3D matter
        self.init_p3d_matter()
        # Initialize bias and fit parameters
        try:
            self.init_bias_and_fit_pars(use_aip15_extrapolation)
        except:
            print("Cannot initialize bias parameters")
    ...
    def init_bias_and_fit_pars(self, use_aip15_extrapolation = False):
        """
        Initialize the D(k, mu) parameters
        from a table.
        """
        if use_aip15_extrapolation:
            for p in ["q1", "q2", "kp", "kvav", "av", "bv", "beta", "bF"]:
                if p != "bF":
                    pval = self.aip15_extrapolation[p]["a"] * self.z + self.aip15_extrapolation[p]["b"]
                else:
                    pval = self.aip15_extrapolation["bTauDelta"]["a"] * self.z + self.aip15_extrapolation["bTauDelta"]["b"]
                    pval *= np.log(self.meanTransmission)
                setattr(self, p, pval)
    ...
    def get_npd13_p1d(self, kPara):
        ...
        k0 = k0 * self.cambResults.hubble_parameter(self.z) / (1 + self.z) / (self.cambResults.hubble_parameter(0)/100)
        kP_over_pi = AF * ((1+self.z)/(1+z0))**BF * (kPara/k0)**(3+nF+aF*np.log(kPara/k0)+bF*np.log((1+self.z)/(1+z0)))

        return kP_over_pi / kPara * np.pi

    def get_P1D(self, kPara, use_npd13_template = False):
        """
        ...
        """
        if use_npd13_template:
            return self.get_npd13_p1d(kPara)
        else:
    ...
    def dP1D_dDelta(self, kPara, use_chiang17_extrapolation = False, zref = "3.0"):
        """
        Compute the response of the
        ...
        if use_chiang17_extrapolation:
            result = self.logp1d_response(zref = "3.0")(kPara)
            result *= self.get_P1D(kPara, use_npd13_template = True)
        else:
            mu = lambda kPerp: kPara / np.sqrt(kPara**2 + kPerp**2)
...
### Part of the code that use the extrapolation
### of Chiang+17 to get the response
    def load_logp1d_response_ratio_pars(self, zref="3.0"):

        path_to_pkl = "chiang17_response_extrapolation/chiang17_response_extrapolation_pars.pkl"
        pars = pickle.load(open(path_to_pkl, "rb"))
        pars = pars[zref]

        return pars

    def model(self, k, zref, **kwargs):
        X = (1 + self.z) / (1 + float(zref))
        a_of_z = kwargs["gamma"] * (X - 1) * X ** kwargs["eta"]
        b_of_z = kwargs["alpha"] * X - kwargs["alpha"]

        return k ** a_of_z * np.exp(b_of_z)

    def logp1d_response(self, zref = "3.0"):

        path_to_pkl = "chiang17_response_extrapolation/chiang17_logp1d_response.pkl"
        response = pickle.load(open(path_to_pkl, "rb"))
        k_para = response["k_para"]
        logp1d_response_ref = response[zref]

        pars = self.load_logp1d_response_ratio_pars(zref)

        logp1d_response = self.model(k_para, zref, **pars) * logp1d_response_ref

        logp1d_response_interp = interp1d(k_para, logp1d_response)
        return logp1d_response_interp
